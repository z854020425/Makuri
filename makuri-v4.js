...
class VirtualList{
	...
	add_styles(){
		Utils.add_styles([
			...
			'.info_singer{width:20%; color:orange; min-width:fit-content; user-select:text; cursor:pointer;}',
			'.info_lang{width:10%; color:grey; min-width:fit-content;}',
			'.info_tags{width:40%; display:flex; justify-content:Wflex-start;}',
			'.info_tag{margin:0px 0.125rem; padding:0px 0.125rem; border:0.125rem dashed gray; border-radius:40% 0%; background:#99e4f6d1; color:blue; min-width:fit-content;}',
			'span.info_tag:not(.info_author){border-color:#0026e5cc;}',
			'span.info_author{min-width:4rem;}',
			'span.真栗{color:chocolate;text-shadow:0 0 0.125rem orange; background:lightyellow; border-color:#f1b11b !important;}',
			...
			'span.🚫{background:#fcd2d2ad; border-color:#fc2927f0 !important;}',
			...
			'span.🤣, span.😭{background:lightyellow; border-color:#f99321 !important;}',
			'span.🌛{background:#154f938f; border-color:#5c51fb !important;}',
			'span.儿歌{background:#00ff6d29; color:green; border-color:#00a100 !important;}',
	...
	render_visible_rows(start, end){
		...
		this.clips_arr.slice(start, end).forEach(song => {
			...
			info_singer.addEventListener('click', (e) => {
				if (window.getSelection().toString().length > 0) return;
				let inp_search = document.querySelector('input.input_search');
				if (inp_search === null) return;
				inp_search.value = `singer:${e.target.getAttribute('title')}`;
				inp_search.dispatchEvent(new Event('search'));
			});
...
class SearchBox{
	...
	mount(){
		...
		let items = [
			...
			// ['晚安🌛专场', 'title:晚安喵|今晩はお月さん|Littlestar|"有可能的夜晚"|兔子先生|香格里拉|"你"|"宝贝"|"ルル"|旅行的意义|Nico|銀色飛行船|"有梦好甜蜜"|LostStars|少しだけ長い冬休み|사랑해쏭', 'special'],
			['晚安🌛专场', 'tag:🌛', 'special'],
...
class Drawers{
	...
	mount(){
		Utils.add_styles([
			...
			'#div_btn_lb div{cursor:pointer; opacity:0.3; font-size:1rem; text-align:center;border:0px solid black;height:3rem; width:3rem; background:lightgray; user-select:none; position:relative; transition:0.3s; font-weight:800;}',
			'#div_btn_lb div:hover{color:#fe9111; background:#ffe999ad; opacity:1;}',
			...
			'.div_btn svg rect{fill:none; width:3rem; height:3rem; stroke-width:4; stroke:gray; stroke-dasharray:12rem; stroke-dashoffset:0; transition: stroke-dashoffset 2.5s ease;}',
			'.div_btn:hover svg rect{stroke-dasharray:12rem; stroke-dashoffset:24rem; stroke:#fe9111;}'
		]);
...
async function main(){
	...
	const TAGS = {
	"🚫": ['百万个吻', '骗赖', '你跟我比夹夹', '嘉宾', '香水有毒', '纤夫的爱', '天上掉下个猪八戒', '通天大道宽又阔', '大哥欢迎你', '好汉歌', '求佛'],
	"🤣": ['百万个吻', '骗赖', '香水有毒', '通天大道宽又阔', '你跟我比夹夹', 'TMD我爱你', '闹啥子嘛闹', '810975', '忐忑', '蕉蕉', '帮主夫人', '史上最红小无常', '天上掉下个猪八戒', 'セーラー服を脱がさないで'],
	"儿歌": ['小鲤鱼历险记', '我爱洗澡', '勇气大爆发', '我会自己上厕所', '加油鸭', '巴啦啦小魔仙', '小小鹿', '别看我是一只羊', '宝贝宝贝', '白龙马', '葫芦娃', '大家一起喜羊羊', '天上掉下个猪八戒', '快乐小孩', '少年英雄小哪吒', '我为厨艺狂', '永远的奥特曼'],
	"😭": ['嘉宾', '阿拉斯加海湾', '就让这大雨全都落下', '心做し', '雨爱', '成全', '连名带姓', '有一种悲伤', '僕が死のうと思ったのは', '北极星的眼泪', '到此为止', '可惜不是你', '刻在我心底的名字', '空心', '浪费', '你瞒我瞒', '人质', '傻子', '说谎', '说散就散', '兔子先生', '我很快乐', '想自由', '一个人的北京', 'カワキヲアメク', '大眠', '你就不要想起我', '会呼吸的痛', '淘汰', '最熟悉的陌生人', '爱我别走', '自傷無色', '走在冷风中', '泡沫', '体面', '失落沙洲', '我怀念的', '我的一个道姑朋友', "吃泡面"],
	"🌛": ['晚安喵', '今晩はお月さん', 'Littlestar', '有可能的夜晚', '兔子先生', '香格里拉', '你', '宝贝', 'ルル', '旅行的意义', 'Nico', '銀色飛行船', '"有梦好甜蜜"', 'LostStars', '少しだけ長い冬休み', '사랑해쏭', '계륵']
	};
